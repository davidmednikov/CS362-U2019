...
int minionAction(int currentPlayer, int getCoins, int discardHand, struct gameState *state, int handPos) {
  ...
  else if (discardHand) //discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
  {
    // discard user hand and hands of players with > 4 cards
    int i;
    for (i = 0; i < state->numPlayers; i++)
    {
      if (state->handCount[i] > 4 && i == currentPlayer)
      {
        //discard hand
...
int tributeAction(int currentPlayer, int nextPlayer, int tributeRevealedCards[], struct gameState *state) {
  ...
  if ((nextPlayerDiscardCount + nextPlayerDeckCount) <= 1) {
    if (nextPlayerDeckCount > 0) {
      tributeRevealedCards[0] = nextPlayerDeck[nextPlayerDeckCount];
      nextPlayerDeckCount--;
    }
    else if (nextPlayerDiscardCount > 0) {
      tributeRevealedCards[0] = state->discard[nextPlayer][nextPlayerDiscardCount - 1];
      nextPlayerDiscardCount--;
    }
    else {
      //No Card to Reveal
      if (DEBUG) {
    ...
    if (nextPlayerDeckCount == 0) {
      int i;
      for (i = 0; i < nextPlayerDiscardCount; i++) {
        nextPlayerDeck[i] = state->discard[nextPlayer][i]; //Move to deck
        nextPlayerDeckCount++;
    ...
    int i;
    for (i = 0; i < 2; i++) {
      tributeRevealedCards[i] = nextPlayerDeck[nextPlayerDeckCount - 1];
      nextPlayerDeck[nextPlayerDeckCount--] = -1;
      nextPlayerDeckCount--;
    }
  }
...
int mineAction(int currentPlayer, int trashTreasure, int gainTreasure, struct gameState *state, int handPos) {
  if ((trashTreasure < copper || trashTreasure > gold) && (gainTreasure < copper || gainTreasure > gold)) {
    return -1;
  }
  ...
  int i;
  for (i = 0; i < state->handCount[currentPlayer]; i++) {
    if (state->hand[currentPlayer][i] == trashTreasure)   {
      discardCard(i, currentPlayer, state, 1);
      break;
    }
  }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  case mine:
    ...
    return mineAction(currentPlayer, j, choice2, state, handPos);
  ...
  case baron:
    return baronAction(currentPlayer, choice1, state);
  ...
  case tribute:
    return tributeAction(currentPlayer, nextPlayer, tributeRevealedCards, state);

  case ambassador:
    return ambassadorAction(currentPlayer, choice1, choice2, state, handPos);
